...
CIFAR10_MODELS = ('resnet18', 'mobilenetv2', 'wrn_16_8', 'wrn_28_10', 'wrn_28_10_ln')
CIFAR100_MODELS = ('resnet18', 'wrn_16_8', 'cifar_wrn_28_10', 'cifar_wrn_28_10_ln')
...
def build_loader(data, data_path, image_size, batch_size, num_workers):
    ...
    if data == 'imagenet100':
        ...
        tf = T.Compose([T.Resize(resize), T.CenterCrop(image_size), T.ToTensor(), T.Normalize(IMAGENET_MEAN, IMAGENET_STD)])
...
def build_model(data, model, num_classes, norm):
    if data == 'cifar10':
        ...
        raise ValueError('unknown data: ' + data)
    return (get_model(model, num_classes=num_classes, norm=norm), load_checkpoint)
...
def main():
    ...
    if args.model.lower() not in allowed[args.data]:
        raise ValueError('model ' + args.model + ' not supported for ' + args.data + '; choose from ' + str(allowed[args.data]))
    ...
    norm = 'ln' if args.model.lower().endswith('_ln') else args.norm
    (model, load_checkpoint) = build_model(args.data, args.model, num_classes, norm)
    model = load_checkpoint(model, args.checkpoint)
    model = model.to(device)
    (loader, _) = build_loader(args.data, args.data_path, args.image_size, args.batch_size, args.num_workers)
